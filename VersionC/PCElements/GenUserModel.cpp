...
void TGenUserModel::Set_Name(const String Value)
{
	...
	else
	{
		...
		FuncError = false;
		FNew = (GenUserModelProc__0) CheckFuncError(GetProcAddress((HMODULE) FHandle, ("New")), "New");
		if(!FuncError)
			FSelect = (GenUserModelProc__2) CheckFuncError(GetProcAddress((HMODULE) FHandle, ("Select")), "Select");
		if(!FuncError)
			FInit = (GenUserModelProc__4) CheckFuncError(GetProcAddress((HMODULE) FHandle, ("Init")), "Init");
		if(!FuncError)
			FCalc = (GenUserModelProc__5) CheckFuncError(GetProcAddress((HMODULE) FHandle, ("Calc")), "Calc");
		if(!FuncError)
			FIntegrate = (GenUserModelProc__6) CheckFuncError(GetProcAddress((HMODULE) FHandle, ("Integrate")), "Integrate");
		if(!FuncError)
			FSave = (GenUserModelProc__8) CheckFuncError(GetProcAddress((HMODULE) FHandle, ("Save")), "Save");
		if(!FuncError)
			fRestore = (GenUserModelProc__9) CheckFuncError(GetProcAddress((HMODULE) FHandle, ("Restore")), "Restore");
		if(!FuncError)
			FEdit = (GenUserModelProc__3) CheckFuncError(GetProcAddress((HMODULE) FHandle, ("Edit")), "Edit");
		if(!FuncError)
			FUpdateModel = (GenUserModelProc__7) CheckFuncError(GetProcAddress((HMODULE) FHandle, ("UpdateMode")), "UpdateMode");
		if(!FuncError)
			FDelete = (GenUserModelProc__1) CheckFuncError(GetProcAddress((HMODULE) FHandle, ("Delete")), "Delete");
		if(!FuncError)
			FNumVars = (GenUserModelProc__10) CheckFuncError(GetProcAddress((HMODULE) FHandle, ("NumVars")), "NumVars");
		if(!FuncError)
			FGetAllVars = (GenUserModelProc__11) CheckFuncError(GetProcAddress((HMODULE) FHandle, ("GetAllVars")), "GetAllVars");
		if(!FuncError)
			FGetVariable = (GenUserModelProc__12) CheckFuncError(GetProcAddress((HMODULE) FHandle, ("GetVariable")), "GetVariable");
		if(!FuncError)
			FSetVariable = (GenUserModelProc__13) CheckFuncError(GetProcAddress((HMODULE) FHandle, ("SetVariable")), "SetVariable");
		if(!FuncError)
			FGetVarName = (GenUserModelProc__14) CheckFuncError(GetProcAddress((HMODULE) FHandle, ("GetVarName")), "GetVarName");
		if(FuncError)
		{
			FreeLibrary((HMODULE) FHandle);
			FID = 0;
