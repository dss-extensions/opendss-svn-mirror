...
//        if FD.Separate then EndInstance (SshPrf, 'PowerElectronicsConnection');
        ...
        begin
            ...
            begin
                ...
                if pInv.DERNameList.Count < 1 then
                begin
                    pBat := ActiveCircuit[ActiveActor].StorageElements.First;
                    while pBat <> nil do
                    begin
                        if pBat.Enabled then
                            RefNode(DynPrf, 'DERIEEEType1.PowerElectronicsConnection', pBat);
                        pBat := ActiveCircuit[ActiveActor].StorageElements.Next;
                    end;
                    pPV := ActiveCircuit[ActiveActor].PVSystems.First;
                    while pPV <> nil do
                    begin
                        if pPV.Enabled then
                            RefNode(DynPrf, 'DERIEEEType1.PowerElectronicsConnection', pPV);
                        pPV := ActiveCircuit[ActiveActor].PVSystems.Next;
                    end;
                end
                else
                begin
                    for i := 1 to pInv.DERNameList.Count do
                    begin
