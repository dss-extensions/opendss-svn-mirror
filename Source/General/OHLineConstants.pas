...
TOHLineConstants = class(TObject)
  private
      FNumConds      :Integer;
      FNumPhases     :Integer;
      FX             :pDoubleArray;
...
procedure TOHLineConstants.Calc(f: double);
...
function TOHLineConstants.Get_Rdc(i, units: Integer): Double;
begin
     Result := FRDC^[i] * From_per_Meter(Units);
end;

function TOHLineConstants.Get_X(i, units: Integer): Double;
begin
    Result := FX^[i] * From_Meters(Units);
end;

function TOHLineConstants.Get_Y(i, units: Integer): Double;
begin
    Result := FY^[i] * From_Meters(Units);
end;

function TOHLineConstants.Get_YCmatrix(f, Lngth: double;
...
procedure TOHLineConstants.Kron(Norder: Integer);
...
procedure TOHLineConstants.Reduce;
...
procedure TOHLineConstants.Set_Frequency(const Value: Double);
begin
     FFrequency := Value;
     Fw := twopi * FFrequency;
...
procedure TOHLineConstants.Set_Rdc(i, units: Integer; const Value: Double);
begin
    If (i>0) and (i<=FNumConds) Then FRdc^[i] := Value * To_per_Meter(units);
end;

procedure TOHLineConstants.Set_X(i, units: Integer; const Value: Double);
begin
    If (i>0) and (i<=FNumConds) Then FX^[i] := Value * To_Meters(units);
end;

procedure TOHLineConstants.Set_Y(i, units: Integer; const Value: Double);
begin
