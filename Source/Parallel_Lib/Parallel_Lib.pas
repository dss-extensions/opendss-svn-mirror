...
begin
    ...
    begin
        case Buffer[i].Relationship of
            ...
            RelationCache:
            begin
                if (Buffer[i].Cache.Level = 1) then
                    Inc(result.ProcessorL1CacheCount)
                else
                if (Buffer[i].Cache.Level = 2) then
                    Inc(result.ProcessorL2CacheCount)
                else
                if (Buffer[i].Cache.Level = 3) then
                    Inc(result.ProcessorL3CacheCount);
            end;
            RelationProcessorPackage:
                Inc(result.ProcessorPackageCount);
        else
...
begin
    ...
    case InfoType of
        NumCPU:
            Result := LProcInfo.LogicalProcessorCount; // gets the number of CPUs (Threads)
        NumCore:
            Result := LProcInfo.ProcessorCoreCount;    // gets the number of physical cores
        NumSocket:
            Result := LProcInfo.NumaNodeCount;         // Number of NUMA Nodes (Should be the same number of sockets)
    end;
end;
