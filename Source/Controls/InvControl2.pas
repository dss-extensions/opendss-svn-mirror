...
//                       CRLF+'the inverter (set in the PVSystem2/Storage2).';
    ...
    PropertyHelp[18] := 'Required for VOLTWATT and VOLTVAR mode.  Must be one of: {INACTIVE* | LPF | RISEFALL }.  The default is INACTIVE.  ' + CRLF + CRLF +
        'Auxiliary option that aims to limit the changes of the desired reactive power and the active power limit between time steps, the alternatives are listed below: ' + CRLF + CRLF + 'INACTIVE. It indicates there is no limit on rate of change imposed for either active or reactive power output. ' + CRLF + CRLF + 'LPF. A low-pass RC filter is applied to the desired reactive power and/or the active power limit to determine the output power as a function of a time constant defined in the LPFTau property. ' + CRLF + CRLF + 'RISEFALL. A rise and fall limit in the change of active and/or reactive power expressed in terms of pu power per second, defined in the RiseFallLimit, is applied to the desired reactive power and/or the active power limit. ';
...
//                    end
                ...
                6:
                begin
                    if CompareTextShortest(Parser[ActorID].StrValue, 'rated') = 0 then
                        FVoltage_CurveX_ref := 0
                    else
                    if CompareTextShortest(Parser[ActorID].StrValue, 'avg') = 0 then
                        FVoltage_CurveX_ref := 1
                    else
                    if CompareTextShortest(Parser[ActorID].StrValue, 'ravg') = 0 then
                        FVoltage_CurveX_ref := 2
                end;
                ...
                11:
                    FArGraLowV := Parser[ActorID].DblValue;
                12:
                    FArGraHiV := Parser[ActorID].DblValue;
                13:
                    FDRCRollAvgWindowLength := InterpretDRCAvgVWindowLen(Param);
                14:
                    FdeltaQ_factor := Parser[ActorID].DblValue;
                15:
                    FVoltageChangeTolerance := Parser[ActorID].DblValue;
                16:
                    FVarChangeTolerance := Parser[ActorID].DblValue;

                17:
                begin
                    if CompareTextShortest(Parser[ActorID].StrValue, 'pavailablepu') = 0 then
                        FVoltwattYAxis := 0
                    else
                    if CompareTextShortest(Parser[ActorID].StrValue, 'pmpppu') = 0 then
                        FVoltwattYAxis := 1
                    else
                    if CompareTextShortest(Parser[ActorID].StrValue, 'pctpmpppu') = 0 then
                        FVoltwattYAxis := 2
                    else
                    if CompareTextShortest(Parser[ActorID].StrValue, 'kvaratingpu') = 0 then
                        FVoltwattYAxis := 3
                end;

                18:
                begin
                    if CompareTextShortest(Parser[ActorID].StrValue, 'inactive') = 0 then
                        RateofChangeMode := INACTIVE
                    else
                    if CompareTextShortest(Parser[ActorID].StrValue, 'lpf') = 0 then
                        RateofChangeMode := LPF
                    else
                    if CompareTextShortest(Parser[ActorID].StrValue, 'risefall') = 0 then
                ...
                begin
                    if Parser[ActorID].DblValue > 0 then
                        FLPFTau := Parser[ActorID].DblValue
                    else
                        RateofChangeMode := INACTIVE;
                end;
                ...
                begin
                    if Parser[ActorID].DblValue > 0 then
                        FRiseFallLimit := Parser[ActorID].DblValue
                    else
                        RateofChangeMode := INACTIVE;
                end;

                21:
                    FdeltaP_factor := Parser[ActorID].DblValue;
                22:
                    ShowEventLog := InterpretYesNo(param);

                23:
                begin
                    if CompareTextShortest(Parser[ActorID].StrValue, 'varaval') = 0 then
                        FReacPower_ref := 'VARAVAL'
                    else
                    if CompareTextShortest(Parser[ActorID].StrValue, 'varmax') = 0 then
                        FReacPower_ref := 'VARMAX'
                end;

                24:
                    FActivePChangeTolerance := Parser[ActorID].DblValue;
                ...
                begin
                    ...
                    if CompareTextShortest(param, 'max') = 0 then
                        FMonBusesPhase := MAXPHASE
                    else
                    if CompareTextShortest(param, 'min') = 0 then
                        FMonBusesPhase := MINPHASE
                    else
                        FMonBusesPhase := max(1, Parser[ActorID].IntValue);
                end;
                ...
                begin //FMonBuses     := Param;
                    ...
                    for i := 0 to FMonBusesNameList.Count - 1 do
                    begin
                ...
                begin
                    ...
                    if CompareTextShortest(StrTemp, 'varaval') = 0 then
                        FReacPower_ref := 'VARAVAL'
                    else
                    if CompareTextShortest(StrTemp, 'varmax') = 0 then
                        FReacPower_ref := 'VARMAX'
                end;
...
begin
    ...
    if OtherInvControl2 <> nil then
        ...
        begin
            ...
            ReallocMem(FMonBusesVbase, SizeOf(FMonBusesVbase^[1]) * FMonBusesNameList.Count);
            for j := 1 to FMonBusesNameList.Count do
                FMonBusesVbase^[j] := OtherInvControl2.FMonBusesVbase^[j];

            TimeDelay := OtherInvControl2.TimeDelay;
            for j := 1 to ParentClass.NumProperties do
                PropertyValue[j] := OtherInvControl2.PropertyValue[j];

        end
    else
        DoSimpleMsg('Error in InvControl2 MakeLike: "' + InvControl2Name + '" Not Found.', 370);
...
begin
    ...
    begin
        ...
        begin
            ...
            if ControlledElement[i].DSSClassName = 'Storage2' then
                TStorage2Obj(ControlledElement[i]).Storage2Vars.PF_Priority := false;
        end;
...
begin
    ...
    begin
        ...
        begin
            ...
            begin
                ...
                if QDesiredVV[k] >= 0.0 then
                    Qoutputpu[k] := TPVSystem2Obj(DERelem).Presentkvar / QHeadroom[k]
                else
                    Qoutputpu[k] := TPVSystem2Obj(DERelem).Presentkvar / QHeadroomNeg[k];
            end
            else
            begin
                ...
                if QDesiredVV[k] >= 0.0 then
                    Qoutputpu[k] := TStorage2Obj(DERelem).Presentkvar / QHeadroom[k]
                else
                    Qoutputpu[k] := TStorage2Obj(DERelem).Presentkvar / QHeadroomNeg[k];
            end;
            ...
            begin
                ...
                if ShowEventLog then
                    AppendtoEventLog('InvControl2.' + Self.Name + ', ' + TPVSystem2Obj(DERelem).QualifiedName,
            ...
            begin
                ...
                if ShowEventLog then
                    AppendtoEventLog('InvControl2.' + Self.Name + ', ' + TStorage2Obj(DERelem).QualifiedName,
        ...
        begin
            ...
            if ControlledElement[k].DSSClassName = 'PVSystem2' then
                TPVSystem2Obj(DERelem).pf_wp_nominal := pf_wp_nominal
            else
                TStorage2Obj(DERelem).kvarRequested := QDesiredWP[k];

            // Sets PVSystem2/Storage2's kvar_out
            if ControlledElement[k].DSSClassName = 'PVSystem2' then
                TPVSystem2Obj(DERelem).Presentkvar := QDesiredWP[k]
            else
                TStorage2Obj(DERelem).kvarRequested := QDesiredWP[k];
        ...
        begin
            ...
            // Sets PVSystem2/Storage2's kvar_out
            if ControlledElement[k].DSSClassName = 'PVSystem2' then
                TPVSystem2Obj(DERelem).Presentkvar := QDesiredWV[k]
            else
                TStorage2Obj(DERelem).kvarRequested := QDesiredWV[k];
        ...
        begin
            ...
            // Sets DER kvar_out
            if ControlledElement[k].DSSClassName = 'PVSystem2' then
                TPVSystem2Obj(DERelem).Presentkvar := QDesiredDRC[k]
            else
                TStorage2Obj(DERelem).kvarRequested := QDesiredDRC[k];
        ...
        begin
            ...
            // Sets DER kvar_out
            if ControlledElement[k].DSSClassName = 'PVSystem2' then
                TPVSystem2Obj(DERelem).Presentkvar := QDesiredVVDRC[k]
            else
                TStorage2Obj(DERelem).kvarRequested := QDesiredVVDRC[k];
        ...
        begin
            ...
            if ControlledElement[k].DSSClassName = 'PVSystem2' then
            begin
                if ((abs(PLimitVW[k]) > 0.0) and (abs(TPVSystem2Obj(DERelem).presentkW - PLimitVW[k]) / PLimitVW[k] > 0.0001)) then
            ...
            begin
                if abs(abs(TStorage2Obj(DERelem).presentkW) - PLimitVW[k]) / PLimitVW[k] > 0.0001 then
                    FVWOperation[k] := 0; // 0.01% is the value chosen at the moment
        ...
        begin
            ...
            if ControlledElement[k].DSSClassName = 'PVSystem2' then
            begin
                if abs(TPVSystem2Obj(DERelem).presentkW - PLimitVW[k]) / PLimitVW[k] > 0.0001 then
                    FVWOperation[k] := 0; // 0.01% is the value chosen at the moment
...
begin
    ...
    begin
        ...
        begin
            ...
            if ControlledElement[i].DSSClassName = 'PVSystem2' then
                PVSys := ControlledElement[i] as TPVSystem2Obj
            else
                Storage2 := ControlledElement[i] as TStorage2Obj;
            ...
            if (FVoltage_CurveX_ref = 2) and (FRollAvgWindow[i].Get_AvgVal <> 0.0) then
            ...
            begin
                ...
                begin
                    ...
                    begin
                        ...
                        if ShowEventLog then
                            AppendtoEventLog('InvControl2.' + Self.Name + ', ' + ControlledElement[i].QualifiedName,
...
begin
    ...
    begin    // Name list is defined - Use it
        ...
        begin
            ...
            if StripExtension(LowerCase(FDERNameList.Strings[i - 1])) = 'storage2' then
            begin
                Storage2 := Storage2Class.Find(StripClassName(FDERNameList.Strings[i - 1]));

                if Assigned(Storage2) then
                begin
                    if Storage2.Enabled then
                        FDERPointerList.New := Storage2
                end
                else
                begin
...
//        end;
        ...
        28:
            Result := Format('%s', [FvoltwattCH_curvename]);
...
begin
    ...
    begin
        ...
        if ControlledElement[j].DSSClassName = 'PVSystem2' then
            PVSys := ControlledElement[j] as TPVSystem2Obj
        else
            Storage2 := ControlledElement[j] as TStorage2Obj;
        ...
        if ControlledElement[j].DSSClassName = 'PVSystem2' then
            ...
            Storage2.VWMode := false;

        if ControlledElement[j].DSSClassName = 'PVSystem2' then
            PVSys.VVmode := false
        else
            Storage2.VVMode := false;

        if ControlledElement[j].DSSClassName = 'PVSystem2' then
            PVSys.DRCmode := false
        else
            Storage2.DRCMode := false;
...
begin
    ...
    begin
        if (FVoltwattYaxis = 0) then
            PBase[j] := TStorage2Obj(DERelem).DCkW * FEffFactor[j]
...
begin
    ...
    begin
        ...
        if (RF_desiredpu - FPriorQDesireOptionpu[m]) < (-1 * FRiseFallLimit * ActiveCircuit[ActorID].Solution.DynaVars.h) then
            QDesireOptionpu[m] := FPriorQDesireOptionpu[m] - FRiseFallLimit * ActiveCircuit[ActorID].Solution.DynaVars.h
        else
    ...
    begin
        ...
        if (RF_desiredpu - FPriorPLimitOptionpu[m]) < (-1 * FRiseFallLimit * ActiveCircuit[ActorID].Solution.DynaVars.h) then
            PLimitOptionpu[m] := FPriorPLimitOptionpu[m] - (FRiseFallLimit * ActiveCircuit[ActorID].Solution.DynaVars.h)
        else
...
begin
    ...
    begin
        ...
        begin
            if TStorage2Obj(ControlledElement[j]).FVWStateRequested then
        ...
        if (TStorage2Obj(ControlledElement[j]).Storage2State = STORE_CHARGING) and (FvoltwattCH_curve <> nil) then
        begin
            if TStorage2Obj(ControlledElement[j]).FVWStateRequested then
                PLimitVWpu[j] := Fvoltwatt_curve.GetYValue(FPresentVpu[j])
            else
...
begin
    ...
    if ((ActiveCircuit[ActorID].Solution.DynaVars.dblHour * 3600.0 / ActiveCircuit[ActorID].Solution.DynaVars.h) < 3.0) then
        voltagechangesolution := 0.0
    else
...
begin
    ...
    if ControlledElement[j].DSSClassName = 'PVSystem2' then
        pf_priority := TPVSystem2Obj(ControlledElement[j]).PVSystem2Vars.PF_Priority
    else
...
begin
    ...
    if (FDRCRollAvgWindow[j].Get_AvgVal / (basekV * 1000.0)) = 0.0 then
        ...
        deltaVDynReac[j] := FPresentDRCVpu[j] - FDRCRollAvgWindow[j].Get_AvgVal / (basekV * 1000.0);
...
begin
    ...
    begin
        ...
        if (runningsumsampletime > VAvgWindowLengthSec) then
            bufferfull := true;
        if (sample.Count = bufferlength) then
            bufferfull := true;
    end;
...
begin
    ...
    if (sample.Count = 0) then
        ...
        Result := runningsumsample / sample.Count;
    {$else}
