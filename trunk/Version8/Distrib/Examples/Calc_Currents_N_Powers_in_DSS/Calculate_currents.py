...
using their Y primitives and voltage at their terminals. 
    ...
    for local_idx in obj_types:
        ...
        for Elm in Elm_list:
            if Elm != '':
                ...
                print(f"Currents for element {DSSCktElm.Name}:")
                Elm_amps    = {}
                Elm_amps['amps']    = np.abs(Elm_amps_c)
                Elm_amps['angle']   = np.angle(Elm_amps_c, deg=True)
                print(pd.DataFrame(Elm_amps))
